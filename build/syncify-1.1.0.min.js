...
}Busy.prototype.toString=function(){return"Busy"};Busy.instance=function(i){return i instanceof Error&&i[KEY]===true};return Busy}(Error)}).call(this)},{}],19:[function(require,module,exports){(function(){var Busy,Executor,MonitorCollector,ParallelExecutor,RootExecutor,SequenceExecutor,current,join_monitors,parallel,reactivity,sequence,util,with_executor,_ref,__hasProp={}.hasOwnProperty,__extends=function(child,parent){for(var key in parent){if(__hasProp.call(parent,key))child[key]=parent[key]}function ctor(){this.constructor=child}ctor.prototype=parent.prototype;child.prototype=new ctor;child.__super__=parent.prototype;return child};util=require("./util");Busy=require("./Busy");reactivity=require("reactivity");MonitorCollector=function(){function MonitorCollector(){this.ms=[]}MonitorCollector.prototype.push=function(m){if(m!=null){return this.ms.push(m)}};MonitorCollector.prototype.join=function(){return join_monitors(this.ms)};MonitorCollector.prototype.empty=function(){return this.ms.length===0};MonitorCollector.prototype.reset=function(){return this.ms=[]};return MonitorCollector}();join_monitors=function(monitors){var cb,fired,m,notifier,_i,_len,_results;if(reactivity.active()&&monitors.length>0){notifier=reactivity.notifier();fired=false;cb=function(){var m,pending_monitors;if(!fired){pending_monitors=function(){var _i,_len,_results;_results=[];for(_i=0,_len=monitors.length;_i<_len;_i++){m=monitors[_i];if(m.state()==="ready"){_results.push(m)}}return _results}();if(pending_monitors.length===0){fired=true;return notifier.change()}}};_results=[];for(_i=0,_len=monitors.length;_i<_len;_i++){m=monitors[_i];_results.push(m.once("change",cb))}return _results}};Executor=function(){function Executor(){}Executor.prototype.run=function(f){var _this=this;return this.parent._run_child(function(){return _this._run(f)})};Executor.prototype._run=function(f){return f()};Executor.prototype._run_child=function(f){return f()};return Executor}();RootExecutor=function(_super){__extends(RootExecutor,_super);function RootExecutor(){_ref=RootExecutor.__super__.constructor.apply(this,arguments);return _ref}RootExecutor.prototype.run=function(f){return f()};return RootExecutor}(Executor);SequenceExecutor=function(_super){__extends(SequenceExecutor,_super);function SequenceExecutor(parent){this.parent=parent}return SequenceExecutor}(Executor);ParallelExecutor=function(_super){__extends(ParallelExecutor,_super);function ParallelExecutor(parent){this.parent=parent;this.monitors=new MonitorCollector;this.pending_count=0}ParallelExecutor.prototype._run=function(f){var res;res=reactivity.run(f);this.monitors.push(res.monitor);this.monitors.join();if(this.pending_count>0){throw new Busy}if(res.error!=null){throw res.error}return res.result};ParallelExecutor.prototype._run_child=function(f){var res;res=reactivity.run(f);this.monitors.push(res.monitor);if(res.error==null){return res.result}if(Busy.instance(res.error)){return this.pending_count++}else{throw res.error}};return ParallelExecutor}(Executor);current=new RootExecutor;with_executor=function(Ex){return function(f){return util.around({before:function(){return current=new Ex(current)},"finally":function(){return current=current.parent},func:function(){var args;args=arguments;return current.run(function(){return f.apply(null,args)})}})}};module.exports={sequence:sequence=with_executor(SequenceExecutor),parallel:parallel=with_executor(ParallelExecutor)}}).call(this)},{"./Busy":18,"./util":24,reactivity:11}],20:[function(require,module,exports){(function(){var Busy,executors,get,mab,mabs,main,pending,reactivity,revert,subscribe,syncify,syncify_global,syncify_local,util,x;reactivity=require("reactivity");util=require("./util");Busy=require("./Busy");mab=require("./memoize_and_block");mabs=require("./memoize_and_block_scope");executors=require("./executors");syncify=function(async_func,opts){var global,hasher;if(opts==null){opts=void 0}global=(opts!=null?opts.global:void 0)===true;hasher=(opts!=null?opts.hasher:void 0)||JSON.stringify;if(global){return syncify_global(async_func,hasher)}else{return syncify_local(async_func,hasher)}};syncify_local=function(async_func,hasher){return function(){var func;if(mabs.defined()){func=mabs.get_or_create(async_func,hasher);return func.apply(null,arguments)}else{throw new Error("local syncified function with no parent context")}}};syncify_global=function(async_func,hasher){return mab(async_func,hasher)};revert=function(func){func=executors.sequence(func);return function(){var args,cb,_ref;_ref=util.args_cb(arguments),args=_ref[0],cb=_ref[1];func=mabs.attach(func);reactivity.subscribe(function(){return func.apply(null,args)},function(e,r,monitor,stopper){if(!Busy.instance(e)){stopper();return typeof cb==="function"?cb(e,r,monitor):void 0}});return void 0}};pending=function(f){var e;try{f();return false}catch(_error){e=_error;if(Busy.instance(e)){return true}else{throw e}}};get=function(f,v){var result;result=void 0;if(pending(function(){return result=f()})){if(typeof v==="function"){return v()}else{return v}}else{return result}};subscribe=function(func,cb){var iter,stopped,stopper;stopped=false;stopper=function(){return stopped=true};(iter=function(){if(!stopped){return revert(func)(function(e,r,m){if(!stopped){if(m!=null){m.on("change",iter)}return typeof cb==="function"?cb(e,r,m,stopper):void 0}})}})();return stopper};main=function(x,y){var args,cb,func;if(arguments.length===3){func=arguments[0],args=arguments[1],cb=arguments[2];args.push(cb);return revert(func).apply(null,args)}else{switch(typeof x+" "+typeof y){case"function undefined":return syncify(x);case"function function":return subscribe(x,y);case"object function":return syncify(y,x);case"function object":return syncify(x,y);default:throw new Error("Invalid Arguments")}}};if((typeof module!=="undefined"&&module!==null?module.exports:void 0)!=null){module.exports=main}x=main;x.revert=revert;x.pending=pending;x.get=get;x.subscribe=subscribe;x.parallel=function(f){return executors.parallel(f)()};x.sequence=function(f){return executors.sequence(f)()};if(typeof window!=="undefined"&&window!==null){window.syncify=main}}).call(this)},{"./Busy":18,"./executors":19,"./memoize_and_block":21,"./memoize_and_block_scope":22,"./util":24,reactivity:11}],21:[function(require,module,exports){(function(){var Busy,executors,reactivity,util,__hasProp={}.hasOwnProperty;util=require("./util");Busy=require("./Busy");reactivity=require("reactivity");executors=require("./executors");module.exports=function(async_func,hasher){var blocked_f,cache;if(hasher==null){hasher=JSON.stringify}cache={};blocked_f=executors.sequence(function(){var args,_name;args=util.arr(arguments);return(cache[_name=hasher(args)]!=null?cache[_name=hasher(args)]:cache[_name]=function(){var c;c=reactivity(new Busy);util.apply(async_func,args,function(e,r){if(e!=null){return c(e)}else{return c(r)}});return c}())()});blocked_f.reset=function(){var cell,k,old_cache,_results;old_cache=cache;cache={};_results=[];for(k in old_cache){if(!__hasProp.call(old_cache,k))continue;cell=old_cache[k];_results.push(cell({}))}return _results};return blocked_f}}).call(this)},{"./Busy":18,"./executors":19,"./util":24,reactivity:11}],22:[function(require,module,exports){(function(){var mab,sr,stack_refmap;stack_refmap=require("./stack_refmap");mab=require("./memoize_and_block");sr=stack_refmap();module.exports={attach:function(func){return sr.attach(func)},get_or_create:function(async_func,hasher){if(hasher==null){hasher=JSON.stringify}return sr.get_or_else(async_func,function(){return mab(async_func,hasher)})},defined:function(){return sr.defined()}}}).call(this)},{"./memoize_and_block":21,"./stack_refmap":23}],23:[function(require,module,exports){(function(){var StackRefmap,refmap,stackval,__bind=function(fn,me){return function(){return fn.apply(me,arguments)}};refmap=require("refmap");stackval=require("stackval");StackRefmap=function(){function StackRefmap(){this.get_or_else=__bind(this.get_or_else,this);this.get=__bind(this.get,this);this.attach=__bind(this.attach,this);this.sv=stackval()}StackRefmap.prototype.attach=function(f){var m;m=refmap();return this.sv.attach(f,function(){return m})};StackRefmap.prototype.get=function(key){return this.sv.get().get(key)};StackRefmap.prototype.get_or_else=function(key,generator){return this.sv.get().get_or_else(key,generator)};StackRefmap.prototype.defined=function(){return this.sv.defined()};return StackRefmap}();module.exports=function(){return new StackRefmap}}).call(this)},{refmap:16,stackval:17}],24:[function(require,module,exports){(function(){var EQ,NOOP,apply,args_cb,around,arr,cast_to_error,concat,delay,get_or_else,hash,log,require_func,run_with_callback,say_hello_delayed;arr=function(arrayish){return Array.prototype.slice.apply(arrayish)};delay=function(){return setTimeout(arguments[1],arguments[0])};concat=function(arr,elm){return arr.concat([elm])};hash=function(v){return JSON.stringify(v)};args_cb=function(args){var cb;args=arr(args);cb=args.pop();if("function"!==typeof cb){throw new Error("Expected a callback function as last argument")}return[args,cb]};apply=function(func,args,cb){if(cb==null){cb=void 0}args=arr(args);if(cb!=null){args=args.concat([cb])}return func.apply(null,args)};run_with_callback=function(f,cb){var e;try{return cb(null,f())}catch(_error){e=_error;return cb(e,null)}};cast_to_error=function(x){if(e instanceof Error){return e}else{return new Error(e)}};get_or_else=function(obj,key,generator){if(key in obj){return obj[key]}else{return obj[key]=generator()}};require_func=function(f){var t;if((t=typeof f)!=="function"){throw new Error("requires a parameter of type 'function', not '"+t+"'")}};say_hello_delayed=function(name,cb){return delay(100,function(){return typeof cb==="function"?cb(null,"Hello "+name):void 0})};log=function(){return console.log.apply(null,arguments)};EQ=function(a,b){return a===b};NOOP=function(){};around=function(opts){return function(){if(typeof opts.before==="function"){opts.before()}try{return opts.func.apply(null,arguments)}finally{if(typeof opts["finally"]==="function"){opts["finally"]()}}}};module.exports={arr:arr,delay:delay,concat:concat,hash:hash,args_cb:args_cb,apply:apply,run_with_callback:run_with_callback,cast_to_error:cast_to_error,get_or_else:get_or_else,require_func:require_func,say_hello_delayed:say_hello_delayed,log:log,EQ:EQ,NOOP:NOOP,around:around}}).call(this)},{}]},{},[20]);